...
(function() {
  ...
  WebExtensions.webRequest.onBeforeSendHeaders.addListener((details) => {
    ...
    if (!isLoaderTab && M3U8_FILTER_REGEXP.exec(url)) {
  ...
  WebExtensions.webRequest.onHeadersReceived.addListener((details) => {
    ...
    if (loaderTabs[tabId] || details.initiator && details.initiator.startsWith("chrome-extension://")) {
      return;
    }
    if (!tab2url[tabId]) {
      return;
    ...
    if (!frame2url[tabId][frameId]) {
      frame2url[tabId][frameId] = details.url;
    }
    if (!videoLists[tabId]) {
      videoLists[tabId] = [];
    }
    ...
    if (cmd === CMD_GET_INFO) {
      ...
      if (cmd === CMD_GET_TITLE) {
        ...
        if (cmd === CMD_WATCH_HEADERS) {
          ...
          if (cmd === CMD_FETCH) {
            const {url, method, headers, resultType, timeout, revokeList} = params;
            revokeObjectURL(tabId, revokeList);
            const init = {method:method ? method : "GET", mode:"cors", credentials:"include", headers};
            fetch(url, init).then((response) => {
              return response.blob();
            }).then((blob) => {
              const blobUrl = URL.createObjectURL(blob);
              storedBlob[tabId][blobUrl] = 1;
  ...
  function revokeObjectURL(tabId, revokeList) {
    if (!storedBlob[tabId]) {
      storedBlob[tabId] = {};
    }
    if (revokeList) {
      for (const url of revokeList) {
        ...
        delete storedBlob[tabId][url];
      }
    } else {
      for (const url in storedBlob[tabId]) {
        URL.revokeObjectURL(url);
      }
      delete storedBlob[tabId];
    }
  }
  function deleteTabInfo(tabId) {
    ...
    let backTo = -1;
    revokeObjectURL(tabId);
    const isLoader = loaderTabs[tabId];
    const isRoot = tabRelation[tabId];
    const savedCookies = cookies[tabId];
