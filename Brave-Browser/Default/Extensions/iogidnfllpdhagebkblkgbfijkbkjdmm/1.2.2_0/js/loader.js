...
(function(globals) {
  ...
  Object.assign(globals, {LM_Fetch, LM_Get, LM_Get_X, LM_Head, LM_ModifyRequest, LM_WatchRequest});
  const WebExtensions = navigator.userAgent.includes("Chrome") ? chrome : browser;
  const isFirefox = navigator.userAgent.includes("Firefox");
  const isEdge = false;
  const SIGNATURE = "loadmonkey";
  let revokeList = [];
  function LM_Fetch(params) {
...
class PlayListLoader {
  ...
  async loadPlayList(testHeaders) {
    ...
    if (chromeVersion < 85) {
      ...
      response = await LM_Fetch({url, method:"GET", headers:testHeaders || headers});
    }
    if (!response.ok) {
      this.retry++;
      if (this.retry >= 2) {
    ...
    try {
      ...
      const res_url = chromeVersion < 85 ? response.url : url;
      if (text.indexOf("#EXTINF:") > 0 || text.indexOf("#EXT-X-TARGETDURATION:") > 0) {
        this.details = await this._handleTrackOrLevelPlaylist(text, res_url, 0);
      } else {
        const data = await this._handleMasterPlaylist(text, res_url, null, null, null);
...
const fetchFragment = async(context, fragment, retryCount, timeout) => {
  ...
  for (let i = 0; i < retryCount; i++) {
    ...
    try {
      let buffer;
      if (chromeVersion < 85) {
        buffer = await LM_Get({url:fragment.url, method:"GET", headers, resultType:"arrayBuffer", timeout});
      } else {
        const response = await LM_Fetch({url:fragment.url, method:"GET", headers});
...
const getMastersList = async(context) => {
  ...
  for (const params of list) {
    ...
    const optionalHeaders = params.addition ? {} : requestHeaders;
    if (params.addition) {
    ...
    const playList = new PlayListLoader;
    let httpStatus = await playList.load(url, optionalHeaders);
    if (httpStatus !== 300) {
      httpStatus = await playList.load(url, requestHeaders);
    }
    if (httpStatus === 300) {
        ...
        for (const level of [...levels, ...audioTracks]) {
          ...
          if (httpStatus === 300) {
            if (subList.isLevelDetail) {
              ...
              level.details.requestHeaders = optionalHeaders;
            } else {
              if (_LOG_) {
