...
if (!window.EXT_AUTOMATION_JS_INSERTED) {
    ...
    chrome.runtime.sendMessage({operation: 'get_automation_setting'}, function (res) {
        ...
        chrome.runtime.onMessage.addListener(function (req, sender, sendResponse) {
            ...
            if (req.operation === 'content_automation_status_storage_limit') {
            ...
            if (req.operation === 'content_automation_media_access') {
                $('body').append($('<iframe>')
                    .attr('allow', 'camera; microphone;')
                    .attr('id', 'nsc_frame_media_access')
                    .attr('style', 'display: none')
                    .attr('src', chrome.runtime.getURL('template/frame-media-access.html?' + req.constraints + '')));
            }
    ...
    window.addEventListener('message', function (e) {
        if (e.data.message  !== undefined && e.data.message.nsc_access !== undefined) $('#nsc_frame_media_access').remove();
    }, false);
}